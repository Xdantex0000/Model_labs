...
namespace Lab7
{
    internal class Model
    {
        ...
        public void TransitConflict(List<Transition> transitions, bool printState)
        {
            ...
            for (int i = 0; i < transitionsByPosition.Length; i++)
                if (transitionsByPosition[i].Count > 1)
                {
                    ...
                    do
                    {
                        result = TransitMany(ref transitionsByPosition[i], ref transited, printState, positions[i]);
                    } while (result);
                }
                else if (!transited.Contains(transitionsByPosition[i][0]) && CheckTransition(transitionsByPosition[i][0]))
                {
                    Transit(transitionsByPosition[i][0], printState);
                    transited.Add(transitionsByPosition[i][0]);
                }
        }
        ...
        public bool TransitMany(ref List<Transition> transitionsByPosition, ref List<Transition> transited, bool printState, Position position)
        {
            double rand = new Random().NextDouble();

            for (int j = 0; j < transitionsByPosition.Count; j++)
            {
                if (rand >= (double)j / (double)transitionsByPosition.Count && rand < (double)(j + 1) / (double)transitionsByPosition.Count)
                {
                    if (position.MarkersCount >= transitionsByPosition[j].InArcs.FirstOrDefault(x => x.NextElement == position).Multiplicity && CheckTransition(transitionsByPosition[j]))
                    {
                        Transit(transitionsByPosition[j], printState);
                        transited.Add(transitionsByPosition[j]);
                        return true;
                    }
                }
            }
        ...
        public bool CheckTransition(Transition transition)
        {
            foreach (var x in transition.InArcs)
            {
                var position = (Position)x.NextElement;
                if (position.MarkersCount < x.Multiplicity)
                    return false;
            }
            return true;
        }
